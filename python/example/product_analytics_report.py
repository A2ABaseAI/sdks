...
async def main():
    ...
    desired_name = "Weekly Product Analytics"
    ...
    if agent is None:
        agent = await client.Agent.create(
            name=desired_name,
            system_prompt=(
                "Fetch product metrics, generate charts, and summarize key trends for the week."
            ),
            mcp_tools=[BaseAITool.WEB_SEARCH_TOOL],
        )
        created = True

    run = await agent.run("Generate this week's product analytics report.", thread)
