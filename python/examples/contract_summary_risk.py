...
async def main():
    ...
    desired_name = "Contract Summary & Risk"
    ...
    if agent is None:
        agent = await client.Agent.create(
            name=desired_name,
            system_prompt=(
                "Summarize contracts, extract key terms, and flag risks with rationale."
            ),
            a2abase_tools=[A2ABaseTools.WEB_SEARCH_TOOL],
        )
        created = True

    run = await agent.run("Summarize the latest NDA and highlight risks.", thread)
