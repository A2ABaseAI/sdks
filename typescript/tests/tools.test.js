...
describe('A2ABaseTool', () => {
    it('should have all expected tool values', () => {
        expect(tools_1.A2ABaseTool.FILES_TOOL).toBe('sb_files_tool');
        expect(tools_1.A2ABaseTool.WEB_SEARCH_TOOL).toBe('web_search_tool');
        expect(tools_1.A2ABaseTool.BROWSER_TOOL).toBe('browser_tool');
    });
    it('should get description for valid tool', () => {
        const desc = (0, tools_1.getAgentPressToolDescription)(tools_1.A2ABaseTool.WEB_SEARCH_TOOL);
        expect(desc).toBe('Search the web for information');
    });
    it('should get description for all tools', () => {
        const tools = Object.values(tools_1.A2ABaseTool);
        for (const tool of tools) {
            const desc = (0, tools_1.getAgentPressToolDescription)(tool);
    ...
    it('should throw error for invalid tool', () => {
        expect(() => {
            (0, tools_1.getAgentPressToolDescription)('invalid_tool');
...
describe('A2ABaseTools type', () => {
    it('should accept A2ABaseTool enum', () => {
        const tool = tools_1.A2ABaseTool.WEB_SEARCH_TOOL;
        expect(tool).toBe(tools_1.A2ABaseTool.WEB_SEARCH_TOOL);
    });
